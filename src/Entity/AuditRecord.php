...
class AuditRecord
{
    ...
    {
        return new self(
            $enforced ? AuditLogEventType::OrganizationTwoFactorEnforcementEnabled : AuditLogEventType::OrganizationTwoFactorEnforcementDisabled,
            [
                'organization' => new OrganizationDisplay((string) $organizationId, $slug, $displayName)->toRecord(),
                'actor' => self::getUserData($actor),
            ],
    ...
    {
        return new self(
            $domains === [] ? AuditLogEventType::OrganizationAllowedEmailDomainsCleared : AuditLogEventType::OrganizationAllowedEmailDomainsSet,
            [
                'organization' => new OrganizationDisplay((string) $organizationId, $slug, $displayName)->toRecord(),
                'domains' => $domains,
                'actor' => self::getUserData($actor),
     ...
     * whether they are shown is {@see \App\Log\Display\AuditLogDisplayFactory}'s call, which keeps them
